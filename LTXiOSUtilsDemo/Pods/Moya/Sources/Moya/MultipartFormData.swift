...
extension RequestMultipartFormData {
    internal func append(data: Data, bodyPart: MultipartFormData) {
        append(data, withName: bodyPart.name, fileName: bodyPart.fileName, mimeType: bodyPart.mimeType)
    }

    internal func append(fileURL url: URL, bodyPart: MultipartFormData) {
    ...
    internal func append(stream: InputStream, length: UInt64, bodyPart: MultipartFormData) {
        append(stream, withLength: length, name: bodyPart.name, fileName: bodyPart.fileName ?? "", mimeType: bodyPart.mimeType ?? "")
    }

    internal func applyMoyaMultipartFormData(_ multipartBody: [Moya.MultipartFormData]) {
